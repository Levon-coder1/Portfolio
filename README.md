...
- Snapshot thumbnails on the home page live in [`images/snapshots`](images/snapshots) and are referenced in [`index.html`](index.html). To swap the SVGs for real photos:
  1. Save your images into `images/snapshots/` (e.g., `projects-shot.jpg`, `gadgets-grid.jpg`).
  2. Update the corresponding `<img src="...">` paths in `index.html` within the “Snapshots” section (Projects, Gadgets, Socials, Hub).
  3. Keep alt text meaningful for accessibility; adjust `styles.css` thumbnail rules if your new assets need different aspect ratios.
